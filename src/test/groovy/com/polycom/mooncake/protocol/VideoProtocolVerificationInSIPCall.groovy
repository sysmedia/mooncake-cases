...
class VideoProtocolVerificationInSIPCall extends MoonCakeSystemTestSpec {
    @Shared
    ...
    @Unroll
    def "Verify Video Protocol In SIP Call With protocol #expectedProtocol And In Call Rate #callRate Kbps"(String videoProtocol,
        ...
        retry(times: 3, delay: 5) {
            moonCake.placeCall(groupSeries, CallType.SIP, callRate)
        }
    ...
    @Ignore
    def "Verify Video Protocol In SIP Call With Specified protocol H264"(String videoProtocol,
                                                                         String expectedProtocol,
                                                                         int[] callRates) {
        setup:
        moonCake.hangUp()
        ...
        then: "MoonCake place call to the GroupSeries with various call rate and then verify the media statistics"
        callRates.each {
            logger.info("===============Start SIP Call with call rate " + it + "===============")
            moonCake.placeCall(hdxIp, CallType.SIP, it)
            ...
            verifyMediaStatistics(MediaChannelType.PVRX, "${expectedProtocol}:--:--:--:--:--")
            verifyMediaStatistics(MediaChannelType.PVTX, "${expectedProtocol}:--:--:--:--:--")
            logger.info("===============Successfully start SIP Call with call rate " + it + "===============")
            moonCake.hangUp()
            pauseTest(5)
        }
