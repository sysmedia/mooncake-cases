 ...
 */
class Auto_Provisioning_Server_Option_Custom_160_To_Custom_Interval extends MoonCakeProvisionTestSpec {
    @Shared
    ...
    def "Verify if the MoonCake will accept new settings from DHCP option 60 when it is in next re-provision interval"() {
        ...
        then: "Hang up the call and wait for next re-provision interval"
        moonCake.hangUp()
        pauseTest(120)
