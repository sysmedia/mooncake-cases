 ...
 * This case is to verify MoonCake should work well in AVC conference when
 ...
 * Conference profile: AVC only, Line rate 4096k + Encryption auto
 */

class Interop_Audio_Mute_MP_AVC extends MoonCakeSystemTestSpec {
    @Shared
    Dma dma
    ...
    @Shared
    String vmr = "4096"   //the vmr number is pre-created on DMA as AVC only conference
    ...
    def "Verify MoonCake call into conference by H323 and mute audio with call rate 2048 or 1024Kbps"() {
        ...
        then: "Set MoonCake and MoonCake_1 H323 registered"
        moonCake.enableH323()
    ...
    def "Verify MoonCake call into conference by SIP and mute audio with call rate 2048 or 1024Kbps"() {
        ...
        then: "Set both MoonCake and MoonCake_1 SIP registered"
        moonCake.enableSIP()
