...
/**
 * This case is to verify the matrix testing with Mooncake by SIP while both endpoints are not
 * registered to any call server
 */

class Interop_Matrix_Mooncake_SIP_Unregistered extends MoonCakeSystemTestSpec {
    ...
    @Unroll
    def "Place call from MoonCake to MoonCake_1 with both SIP unregistered and call rate #CallRate Kbps"(int CallRate,
        ...
        when: "MoonCake place SIP call to MoonCake_1 and Mute MoonCake video"
        logger.info("===============Start SIP Call with call rate " + CallRate + "===============")
        retry(times: 3, delay: 5) {
            moonCake.placeCall(moonCake_1.ip, CallType.SIP, CallRate)
        }
    ...
    @Unroll
    def "Place call from MoonCake_1 to MoonCake with both SIP unregistered and call rate 64 Kbps"() {
        setup:
        moonCake.hangUp()
        moonCake_1.hangUp()
        ...
        when: "MoonCake place SIP call to MoonCake_1 and Mute MoonCake video"
        logger.info("===============Start SIP Call with call rate 64 KBps===============")
        retry(times: 3, delay: 5) {
            moonCake_1.placeCall(moonCake.ip, CallType.SIP, 64)
        }
        ...
        verifyMediaStatistics(MediaChannelType.ATX, "SirenLPR:--:--:--:--:--")
        verifyMediaStatistics(MediaChannelType.ARX, "SirenLPR:--:--:--:--:--")
        logger.info("===============Successfully started SIP Call with call rate 64 KBps===============")
    ...
    @Unroll
    def "Place call from MoonCake_1 to MoonCake with both SIP unregistered and call rate #CallRate Kbps"(int CallRate,
                                                                                                         String expectedProtocol,
                                                                                                         String expectedResolution_PVTX,
                                                                                                         String expectedResolution_CVTX,
                                                                                                         String expectedResolution_PVTX_Sending,
        ...
        logger.info("===============Start SIP Call with call rate " + CallRate + "===============")
        retry(times: 3, delay: 5) {
            moonCake_1.placeCall(moonCake.ip, CallType.SIP, CallRate)
        }
