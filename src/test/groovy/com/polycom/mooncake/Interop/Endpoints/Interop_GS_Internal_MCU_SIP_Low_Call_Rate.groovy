 ...
 */
class Interop_GS_Internal_MCU_SIP_Low_Call_Rate extends MoonCakeSystemTestSpec{
    @Shared
    ...
    def "All EP SIP + All EP the lowest call rate (except audio only call rate) + Encryption on"(){
        ...
        retry(times: 3, delay: 5){
            moonCake.placeCall(gsSipUserName, CallType.SIP, 256)
        }
        pauseTest(3)
        retry(times: 3, delay: 5) {
            rpdWin.placeCall(gsSipUserName, CallType.SIP, 256)
        }
    ...
    def "SUT SIP 768k + Another SUT H.323 768k"(){
        setup: "make sure all endpoint was not in call"
        hangUpAll()
        ...
        then: "rpd place H323, moonCale place Sip with GS internal MCU"
        retry(times: 3, delay: 5){
            moonCake.placeCall(gsSipUserName, CallType.SIP, 768)
        }
        pauseTest(3)
        retry(times: 3, delay: 5) {
            rpdWin.placeCall(gs_e164Num, CallType.H323, 768)
        }
