 ...
 * Provision items: call setting call rate--different call rate when MoonCake as SIP caller.
 * <CallSettings>
 ...
 * </CallSettings>
 */
class FTP_Provisioning_Call_Settings_Network_Call_Rate_SIP_Caller extends MoonCakeProvisionTestSpec {
    @Shared
    ...
    def "Verify if the MoonCake can be set the call rate with the FTP provision and can call with the remote endpoint with rate #callRate KBPS"(int callRate) {
        ...
        retry(times: 5, delay: 5) { moonCake.mediaStatistics.callRate == callRate }
