 ...
 *
 * Verify that video quality is good for AVC and SVC video transcoding in AVC/SVC mixed conference with 4096k 1080p enabled
 * Test AVC 1080p EP + SVC 360p SUT
 * Test MCU can decode AVC 1080p to SVC 360p/180p
 * Test MCU can decode AVC 720p to SVC 360p/180p
 ...
 * Test MCU can decode SVC 360p/180p to AVC 360p/180p
 ...
 * - ENABLE_1080_SVC: YES
 */
class SVC_AVC_Mixed_Mode_4096k_1080p_H323 extends MoonCakeSystemTestSpec{
    @Shared
    Dma dma
    ...
    @Shared
    def callRateLists = [4096 , 1024, 512]
    ...
    def setupSpec() {
        ...
        moonCake.updateCallSettings(4096, "auto", true, false, true);
    ...
    def "Verify MoonCake dial in conference with different resolution for AVC"(String dialString,
        ...
        moonCake.hangUp()
        groupSeries.hangUp()
        moonCake.updateCallSettings(callRate, "auto", true, false, true);
