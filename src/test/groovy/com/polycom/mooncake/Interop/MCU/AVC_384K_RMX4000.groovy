 ...
 *
 * This case is to verify MoonCake calling MCU via VMR with call rate 384K
 *
 * Environment: MCU register H.323 and SIP to DMA, DMA integrated with MCU for both H.323 and SIP
 *              Conference profile: Line rate 384k + Encryption auto
 *
 * Check audio, video,content
 * Test with H323 encryption on/off  and SIP TCP
 */
class AVC_384K_RMX4000 extends MoonCakeSystemTestSpec{
    @Shared
    Dma dma
    ...
    @Shared
    String h323Name = "automooncake384"

    @Shared
    String e164Num = "84381384"

    @Shared
    String sipUri = "mooncake384"
    @Shared
    String expectedResolution_PVRX="640x368"
    @Shared
    ...
    def setupSpec() {
        ...
        dma.createConferenceTemplate(callTmplAVCAuto, "AVC only call template AES Auto", "384", ConferenceCodecSupport.AVC, EncryptionPolicy.ENCRYPTED_PREFERRED)
        dma.createVmr(vmrAVCAuto, callTmplAVCAuto, poolOrderRMX4000NGB, dma.domain, dma.username, null, null)
    ...
    @Unroll
    def "Verify MoonCake Can Join The H323 Conference With Call Rate 384Kbps with Encryption ON/OFF by dialing VMR#dialString"(String dialString,
                                                                                                                               int callRate,
                                                                                                                               String aesSetting) {
        setup:
        moonCake.hangUp()
        groupSeries.hangUp()
        moonCake.enableH323()
        moonCake.updateCallSettings(384, aesSetting, true, false, true)
        ...
        then: "Place call on the mooncake with call rate 384Kbps"
        ...
        verifyMediaStatistics(MediaChannelType.PVRX, "H.264High:$expectedResolution_PVRX:--:--:--:--")
    ...
    def "Verify MoonCake Can Join The SIP Conference With Call Rate 384Kbps by dialing VMR#dialString"(String dialString,
                                                                                             String sipTransProtocol,
                                                                                             int callRate) {
        setup:
        moonCake.enableSIP()
        groupSeries.enableSIP()
        moonCake.updateCallSettings(384, "off", true, false, true);
    ...
    def getTestData_1() {
        def rtn = []
        rtn << [vmrAVCAuto, 384,"on"]
        rtn << [vmrAVCAuto, 384,"off"]
        return rtn
    }
    /**
