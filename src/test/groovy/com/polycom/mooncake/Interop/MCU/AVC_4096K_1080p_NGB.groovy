...
/**
 * Created by Gary Wang on 2019-06-03.
 *
 * This case is to verify MoonCake calling NGB MCU via VMR with call rate 4096K
 *
 * Environment: MCU register H.323 and SIP to DMA, DMA integrated with MCU for both H.323 and SIP
 *              Conference profile: Line rate 4096k + Encryption auto
 *
 * Check audio, video,content
 * Test with H323 and SIP TCP
 */
class AVC_4096K_1080p_NGB extends MoonCakeSystemTestSpec{
    @Shared
    Dma dma
    ...
    def setupSpec() {
        ...
        dma.createConferenceTemplate(callTmplAVCAuto, "AVC only call template AES Auto", "4096", ConferenceCodecSupport.AVC, EncryptionPolicy.ENCRYPTED_PREFERRED)
    ...
    def "Verify MoonCake Can Join The H323 Conference With Call Rate 4096Kbps by dialing VMR#dialString"(String dialString,
                                                                                              int callRate) {
        setup:
        moonCake.hangUp()
        groupSeries.hangUp()
    ...
    def "Verify MoonCake Can Join The SIP Conference With Call Rate 4096Kbps by dialing VMR#dialString"(String dialString,
