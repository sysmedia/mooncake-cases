 ...
 *
 * This case is to verify MoonCake calling MCU via VMR with call rate 512K
 *
 * Environment: MCU register H.323 and SIP to DMA, DMA integrated with MCU for both H.323 and SIP
 *              Conference profile: Line rate 512k + Encryption auto
 *
 * Check audio, video,content
 * Test with H323 encryption on/off  and SIP TCP
 */
class AVC_512K_RMX4000 extends MoonCakeSystemTestSpec{
    @Shared
    Dma dma
    ...
    @Shared
    String h323Name = "automooncake512"

    @Shared
    String e164Num = "84381512"

    @Shared
    String sipUri = "mooncake512"
    @Shared
    ...
    def setupSpec() {
        ...
        dma.createConferenceTemplate(callTmplAVCAuto, "AVC only call template AES Auto", "512", ConferenceCodecSupport.AVC, EncryptionPolicy.ENCRYPTED_PREFERRED)
    ...
    @Unroll
    def "Verify MoonCake Can Join The H323 Conference With Call Rate 512Kbps with Encryption ON/OFF by dialing VMR#dialString"(String dialString,
                                                                                                                               int callRate,
                                                                                                                               String aesSetting) {
        setup:
        moonCake.hangUp()
        groupSeries.hangUp()
        moonCake.enableH323()
        moonCake.updateCallSettings(512, aesSetting, true, false, true)
        ...
        then: "Place call on the mooncake with call rate 512Kbps"
    ...
    def "Verify MoonCake Can Join The SIP Conference With Call Rate 512Kbps by dialing VMR#dialString"(String dialString,
                                                                                             String sipTransProtocol,
                                                                                             int callRate) {
        setup:
        moonCake.enableSIP()
        groupSeries.enableSIP()
        moonCake.updateCallSettings(512, "off", true, false, true);
    ...
    def getTestData_1() {
        def rtn = []
        rtn << [vmrAVCAuto, 512,"on"]
        rtn << [vmrAVCAuto, 512,"off"]
        return rtn
    }
    /**
