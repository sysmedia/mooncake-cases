 ...
 *
 * This case is to verify MoonCake calling MCU via VMR with call rate 768K
 *
 * Environment: MCU register H.323 and SIP to DMA, DMA integrated with MCU for both H.323 and SIP
 *              Conference profile: Line rate 768k + Encryption auto
 *
 * Check audio, video,content
 * Test with H323 encryption on/off  and SIP TCP
 */
class AVC_768K_RMX1800 extends MoonCakeSystemTestSpec{
    @Shared
    Dma dma
    ...
    @Shared
    String h323Name = "automooncake768"

    @Shared
    String e164Num = "84381768"

    @Shared
    String sipUri = "mooncake768"
    @Shared
    String expectedResolution_PVRX="864x480"
    @Shared
    ...
    def setupSpec() {
        ...
        dma.createConferenceTemplate(callTmplAVCAuto, "AVC only call template AES Auto", "768", ConferenceCodecSupport.AVC, EncryptionPolicy.ENCRYPTED_PREFERRED)
    ...
    @Unroll
    def "Verify MoonCake Can Join The H323 Conference With Call Rate 768Kbps with Encryption ON/OFF by dialing VMR#dialString"(String dialString,
                                                                                                                               int callRate,
                                                                                                                               String aesSetting) {
        setup:
        moonCake.hangUp()
        groupSeries.hangUp()
        moonCake.enableH323()
        moonCake.updateCallSettings(768, aesSetting, true, false, true)
        ...
        then: "Place call on the mooncake with call rate 768Kbps"
    ...
    def "Verify MoonCake Can Join The SIP Conference With Call Rate 768Kbps by dialing VMR#dialString"(String dialString,
    ...
    def getTestData_1() {
        def rtn = []
        rtn << [vmrAVCAuto, 768,"on"]
        rtn << [vmrAVCAuto, 768,"off"]
        return rtn
    }
    /**
