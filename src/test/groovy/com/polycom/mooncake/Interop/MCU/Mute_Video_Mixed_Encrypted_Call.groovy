 ...
 *
 #This case is to verify the scenario while MoonCake turn off its video in mixed encrypted call
 ...
 */
class Mute_Video_Mixed_Encrypted_Call extends MoonCakeSystemTestSpec{
    @Shared
    Dma dma
    ...
    def "Verify MoonCake joins encrypted VMR by H323"() {
        setup:
        moonCake.hangUp()
        moonCake111.hangUp()
        groupSeries.hangUp()
   ...
   /* def "Verify MoonCake joins unencrypted VMR by SIP"() {
        ...
        moonCake111.registerSip("TLS", true, "", dma.ip, ep_sip_username, ep_sip_username, "")
        pauseTest(10)
        when: "Place call on the mooncake with call rate 2048Kbps"
        logger.info("===============Start H.323 Call with call rate " + callRate + "===============")
        retry(times: 3, delay: 5) {
