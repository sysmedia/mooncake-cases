 ...
 *
 * This case is to verify MoonCake calling MCU via VMR with call rate 3072K
 *
 * Environment: MCU register H.323 and SIP to DMA, DMA integrated with MCU for both H.323 and SIP
 *              Conference profile: Line rate 3072K + Encryption auto
 *
 * Check audio, video,content
 * Test with H323 and SIP TCP
 */
class AVC_3072K_1080p_RMX1800 extends MoonCakeSystemTestSpec{
    @Shared
    Dma dma
    ...
    def setupSpec() {
        ...
        dma.createConferenceTemplate(callTmplAVCAuto, "AVC only call template AES Auto", "3072", ConferenceCodecSupport.AVC, EncryptionPolicy.ENCRYPTED_PREFERRED)
    ...
    def "Verify MoonCake Can Join The H323 Conference With Call Rate 3072Kbps by dialing VMR#dialString"(String dialString,
                                                                                             int callRate) {
        setup:
        moonCake.hangUp()
        groupSeries.hangUp()
        ...
        then: "Place call on the mooncake with call rate 3072Kbps"
    ...
    def "Verify MoonCake Can Join The SIP Conference With Call Rate 3072Kbps by dialing VMR#dialString"(String dialString,
